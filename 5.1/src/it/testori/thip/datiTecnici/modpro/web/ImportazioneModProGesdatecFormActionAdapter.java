...
public class ImportazioneModProGesdatecFormActionAdapter extends BatchFormActionAdapter {
	...
	protected void saveBatch(ServletEnvironment se, MultipartHandler mh, String url)
		...
		if (tmpFile != null) {
			...
			if (tmpFile != null) {
				url += URLUtils.get().encode("TemporaryFileName") + "=" + URLUtils.get().encode(tmpFile.getAbsolutePath());
				MultipartFile mf = (MultipartFile) mh.getMultipartFiles().get(0);
				url += "&" + URLUtils.get().encode("NomeFileTemp") + "=" + URLUtils.get().encode(mf.getFileName());
			}
		} else {
	...
	private String getQueryString(MultipartHandler mh, String name) {
		...
		for (int i = 0; i < values.length; i++) {
			par += URLUtils.get().encode(name) + "=" + URLUtils.get().encode(values[i]) + "&";
		}
		return par;
	}
	...
	protected File handleFile(ServletEnvironment se, MultipartHandler mh) {
		try {
			...
			if ((mf.getFileName().endsWith("csv"))) {
				se.addErrorMessage(new ErrorMessage("BAS0000078", "File con formato errato \n Formati accettati: .csv"));
				return null;
			}
			// Crea un file temporaneo in cui parcheggiare il file ricevuto
			File tmpFile = File.createTempFile("YImpModProGS", ".xlsx");
