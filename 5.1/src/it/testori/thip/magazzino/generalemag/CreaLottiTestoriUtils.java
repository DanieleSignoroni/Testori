...
public class CreaLottiTestoriUtils {
	...
	public static final String STMT_MAX_ID_PROGRESSIVO_PEZZE = "SELECT MAX(CAST(SUBSTRING(ID_LOTTO, 5, 4) AS INT)) "
			+ "FROM "+LottoTM.TABLE_NAME+" "
			+ "WHERE "+LottoTM.ID_AZIENDA+" = ? "
			+ "  AND SUBSTRING("+LottoTM.ID_LOTTO+", 1, 4) = ? "
			+ "";
	...
	public static String getMaxProgressivoPezzeFromLotto(String idAzienda,String idLotto) throws SQLException {
		String firstPart = idLotto.substring(0,4);
		try (PreparedStatement stmt = ConnectionManager.getCurrentConnection().prepareStatement(STMT_MAX_ID_PROGRESSIVO_PEZZE)) {
			stmt.setString(1, idAzienda);
			stmt.setString(2, idLotto.substring(0,4));

			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					String maxIdLotto = rs.getString(1);
					if(maxIdLotto == null) {
						String suffissoFormattato = String.format("%0" + 4 + "d", 1);
						return suffissoFormattato;
					}else {
						int numero = 0;
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					...
					if (rs.wasNull()) {
						...
						firstPart += String.format("%02d", maxProgressivo + 1);
					}
				}
			}
		}
	...
	public static String getNextLottoProgressivoMonoChar(String idAzienda, String idLottoBase, char tipoTaglio) throws SQLException {
