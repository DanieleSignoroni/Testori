 ...
 * 42234   06/05/2024  TA        Aggiungere i metodi: isQtaUMRifIntera, isQtaUMPrmIntera, isQtaUMSecIntera
 * 42519  03/06/2024   LTB       Errore in fase di evasione righe spese con articoli ad um quantità intera (al interno di dati magaz articolo)		
 * 43265   16/10/2024  SZ		 Viene sostituti il messagi di worning inveci di aggiungere un nuovo.
 * 43937   28/11/2024  LTB       In alcune condizioni di la proposizione atomatica dei lotti occupa in modo abnorme la memoria 
 * 44410   27/12/2024  TA        Anomalia gestione matricole su documento di acquisto in caso di creazione righe da clipboard da ordine acquisto
 * 44468   13/01/2025  SBR       Migliorata gestione anomalie in casi particolari
 * 44796  06/02/2025   Jackal    Aggiunti ganci per personalizzazioni		
*/
  ...
  protected OneToMany getRigheSecondarieInternal() {
    if (iRigheSecondarie.isNew()) {
      iRigheSecondarie.retrieve();
		completaRigheSecondarie(); //43937 
    }
    return iRigheSecondarie;
  }
//43937 inizio
  protected void completaRigheSecondarie() {
	 Iterator iter = iRigheSecondarie.iterator();
	 OrdineAcquistoRigaPrm  rigaOrdinePrm = (OrdineAcquistoRigaPrm)getRigaOrdine();
	 
	 while (iter.hasNext()) {
		 DocumentoAcqRigaSec rigaDocS = (DocumentoAcqRigaSec) iter.next();
	     if (rigaDocS.getRigaOrdine() != null) {
	    	 OrdineAcquistoRigaSec rigaOrdS =   (OrdineAcquistoRigaSec)rigaDocS.getRigaOrdine();
	    	 rigaOrdS.setRigaPrimaria(rigaOrdinePrm);
	     }
	 }
  }
  //43937 fine
  	//Fix Inizio 42234
	public boolean isQtaUMRifIntera() {
		if(getUMRif() != null && getUMRif().getQtaIntera()) 
	       return true; 
		return false;
	  }

	  public boolean isQtaUMPrmIntera() {
		  if((getUMPrm() != null && getUMPrm().getQtaIntera()) || (getArticolo() != null && getArticolo().getArticoloDatiMagaz().isQtaIntera())) 
			  if(getUMPrm() != null) //42519
				  return true;
		  return false;
	  }

	  public boolean isQtaUMSecIntera() {
		  if(getUMSec() != null  && getUMSec().getQtaIntera()) 
		       return true;
		  return false;
	  }
	  
	  public char getTipoArrotondamentoUMRif() {
			if(getUMRif() != null && getUMRif().getQtaIntera()) 
		       return getUMRif().getTipoArrotondamento(); 
			return '-';
		  }
	  
	  public char getTipoArrotondamentoUMPrm() {
		  if((getUMPrm() != null && getUMPrm().getQtaIntera()) || (getArticolo() != null && getArticolo().getArticoloDatiMagaz().isQtaIntera())) 
			  if(getUMPrm() != null) //42519
				  return getUMPrm().getTipoArrotondamento();
		return '-';
	  }
	  
	  public char getTipoArrotondamentoUMSec() {
		  if(getUMSec() != null  && getUMSec().getQtaIntera()) 
			  return getUMSec().getTipoArrotondamento();
		  return '-';
	  }
	  //Fix Fine 42234
  /**
    ...
    // Fine 6652
      ...
      if (riga.getDocEvaAcqRiga().isAddable(riga.getDocEvaAcqRiga(), rigaOrdine)) {
        ...
        if (!doc.isDocumentoDiReso()) {
		  ...
		  riga.setProposizioneAutLotto(null); //43937
          if (emLottiPrm != null) {
            String msg =
            ...
            if (doc.getRigheWarnings() != null) {
            	String oldMsg = "";
                if (doc.getRigheWarnings().containsKey(riga.getRigaOrdine()))
                  oldMsg = (String)doc.getRigheWarnings().get(riga.getRigaOrdine()) + "\n <br/>";
                oldMsg += msg;
                doc.getRigheWarnings().put(riga.getRigaOrdine(), oldMsg);
              }
        ...
        //Fix 6583 - inizio
        //ErrorMessage emDichMatr = riga.getDocEvaAcqRiga().checkDichiarazioneMatricole();//Fix 44410
        ErrorMessage emDichMatr = null;//Fix 44410
        if (!doc.isDaClipboardFactory())//Fix 44410
        	 emDichMatr = riga.getDocEvaAcqRiga().checkDichiarazioneMatricole();//Fix 44410
...
//                      }
                ...
                rigaSec.setProposizioneAutLotto(null); //43937
        ...
        case TipiControlloDisponibilita.GIACENZA: {
          //if (!this.getDocEvaAcqRiga().isDocumentoDiReso()) { // fix 11473
            ...
            if (this.getDocEvaAcqRiga().isAbilitaWarnings() && addWarnings && !msg.trim().equals("")) { // fix 11473
                //Fix 43265 Inizio
                if (((DocEvaAcq)this.getTestata()).getRigheWarnings() != null) {
                	String oldMsg = "";
                    if (( (DocEvaAcq)this.getTestata()).getRigheWarnings().containsKey(this.getRigaOrdine()))
                      oldMsg = (String)( (DocEvaAcq)this.getTestata()).getRigheWarnings().get(this.getRigaOrdine()) + "\n <br/>";
                    oldMsg += msg;
                    ( (DocEvaAcq)this.getTestata()).getRigheWarnings().put(this.getRigaOrdine(), oldMsg);
                  }
    ...
    if (this.isRigaEstratta() && this.isOnDB() && !isCollegataAMagazzino) {
      ...
      if (isCollegataAMagazzino) {
        ...
        if (this.getRigaOrdine() != null) {
            ...
            //( (DocEvaAcq)this.getTestata()).getRigheWarnings().put(this.getRigaOrdine(), message);
            //Fix 43265 Fine
        }
      }
    }
    ...
    try {
      ...
      if (salvaRiga && !this.isCollegataAMagazzino()) {
        ...
        if (this.isCollegataAMagazzino()) {
          ...
          if(ex != null) {
          	String msgErr = ResourceLoader.getString("it.thera.thip.base.comuniVenAcq.resources.ComuniVenAcq", "MsgErr_COLLEGATO_MAG" , new String[] {this.getKey()});
          	ex.setErrorMessage(new ErrorMessage("THIP_BS000", new String[]{msgErr + " \n " + (ex.getMessage() != null ? ex.getMessage() : ex.toString())}));
          }
          //44468
        }
        else {
      ...
      if (t instanceof ThipException) {
        ...
        //ex.setErrorMessage(new ErrorMessage("THIP_BS000"));
        if(ex != null) {
        	ex.setErrorMessage(new ErrorMessage("THIP_BS000", new String[]{ResourceLoader.getString("it.thera.thip.base.comuniVenAcq.resources.ComuniVenAcq", "MsgErr_SALVATAGGIO_RIGA")+ this.getKey() + " \n " + (ex.getMessage() != null ? ex.getMessage() : ex.toString())}));
        }
        //44468
      }
      // fix 11614 <
...
// Fine 3266
  ...
  private static boolean generaRigaSecDocOk(DocEvaAcq docEva, OrdineAcquistoRigaSec ordRigaSec) {
          ...
          if (artRigaSec != null) {
//		  if (artRigaSec.getModalitaPrelievo() == ArticoloDatiProduz.SINGOLA) {		//Fix 44796
		  if (docEva.isControllaEvasioneOrdRigSecFloorstock(ordRigaSec)) {			//Fix 44796
...
//MG FIX 6342 fine
          ...
          if (cauDoc != null) {
              char tipoDoc = cauDoc.getTipoDocumento();
              if (tipoDoc == TipoDocumentoAcq.SPED_LAV_ESN && ((YArticolo)artRigaSec).isFloorStockContoLavoro()) {
            	  ret = false;
              }
          }
          }
  ...
  public ErrorMessage checkArticoloEsclusione() {
	  //Fix 34783 inizio
	  DocEvaAcq testata = (DocEvaAcq)getTestata();
	    //if (testata.isAttivaCheckDichiarazioneMatricole()) {//Fix 44410
	  	if (testata.isAttivaCheckArticoloEsclusione()) {//Fix 44410
