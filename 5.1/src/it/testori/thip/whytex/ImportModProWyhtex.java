...
import com.thera.thermfw.batchload.BatchLoadErrorTM;
import com.thera.thermfw.batchload.BatchLoadValidTM;
...
public class ImportModProWyhtex extends ImportFileWhytex {
	...
	private static final String STMT_ERRORI_CM = "SELECT "
			+ "	"+BatchLoadErrorTM.APPL_KEY_02+", "
			+ "	"+BatchLoadErrorTM.LONG_ERROR+" "
			+ "FROM "
			+ "	"+BatchLoadErrorTM.TABLE_NAME+" "
			+ "WHERE "
			+ "	"+BatchLoadErrorTM.BATCH_JOB_ID+" = ? ";
	public static CachedStatement cErroriCm = new CachedStatement(STMT_ERRORI_CM);

	private static final String STMT_VALID_CM = "SELECT "
			+ "	"+BatchLoadValidTM.APPL_KEY_02+", "
			+ "	"+BatchLoadValidTM.DESCRIPTION+" "
			+ "FROM "
			+ "	"+BatchLoadValidTM.TABLE_NAME+" "
			+ "WHERE "
			+ "	"+BatchLoadValidTM.BATCH_JOB_ID+" = ? ";
	public static CachedStatement cValidCM = new CachedStatement(STMT_VALID_CM);
	...
	protected boolean run() {
		...
		if(ok) {
			if(files != null && files.length == 0) {
				...
				for(File file : files) {
					try {
						CSVFile csvFile = parseFile(file, 2);
						if(csvFile != null) {
				...
				if(numRigheInserite > 0) {
					...
					try {
						...
						if (resSave >= ErrorCodes.NO_ROWS_UPDATED) {
							...
							if(cmSempl.getBatchJob().getStatus() == BatchJob.COMPLETED_ERROR) {
								output.println("Importazione modelli terminata con errore, vedere il lavoro : "+cmSempl.getBatchJob().getBatchJobId());
							}
							List<String> validi = validKeyDescriptionBatchLoadValid(cmSempl.getBatchJob().getBatchJobId());
							if(!validi.isEmpty()) {
								for(String error : validi) {
									output.println(error);
								}
							}
							output.println();
							output.println();
							List<String> errors = erroriKeyLongErrorBatchLoadError(cmSempl.getBatchJob().getBatchJobId());
							if(!errors.isEmpty()) {
								for(String error : errors) {
									output.println(error);
								}
							}
							List<String> modelliValidi = modelliValidiCreati(cmSempl.getBatchJob().getBatchJobId());
							for(String idArticolo : modelliValidi) {
	...
	public static List<String> erroriKeyLongErrorBatchLoadError(int batchJobId){
		List<String> errors = new ArrayList<String>();
		ResultSet rs = null;
		try { 
			PreparedStatement pErroriCm = cErroriCm.getStatement();
			pErroriCm.setInt(1, batchJobId);
			rs = pErroriCm.executeQuery();
			while(rs.next()) {
				errors.add("** Errore "+rs.getString(BatchLoadErrorTM.APPL_KEY_02)+" : "+rs.getString(BatchLoadErrorTM.LONG_ERROR));
			}
	...
	protected static List<String> modelliValidiCreati(int batchJobId){
		List<String> modelli = new ArrayList<String>();
		ResultSet rs = null;
		try { 
			PreparedStatement pErroriCm = cValidCM.getStatement();
			pErroriCm.setInt(1, batchJobId);
			rs = pErroriCm.executeQuery();
			while(rs.next()) {
				modelli.add(rs.getString(BatchLoadValidTM.APPL_KEY_02));
			}
		}catch (SQLException e) {
			e.printStackTrace(Trace.excStream);
		}finally {
	...
	public static List<String> validKeyDescriptionBatchLoadValid(int batchJobId){
		List<String> errors = new ArrayList<String>();
		ResultSet rs = null;
		try { 
			PreparedStatement pErroriCm = cValidCM.getStatement();
			...
			rs = pErroriCm.executeQuery();
			while(rs.next()) {
				errors.add("** OK "+rs.getString(BatchLoadValidTM.APPL_KEY_02)+" : "+rs.getString(BatchLoadValidTM.DESCRIPTION));
			}
		}catch (SQLException e) {
			e.printStackTrace(Trace.excStream);
		}finally {
