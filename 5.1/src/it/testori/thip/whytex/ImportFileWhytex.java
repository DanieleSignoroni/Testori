...
public class ImportFileWhytex extends BatchRunnable implements Authorizable {
	...
	public CSVFile parseFile(File file, int numRigheIntestazione) {
		output.println(" parse del file: "+file.getName());
		...
		if (errori != null && !errori.isEmpty()) {
			...
			output.println(" termine parse file avvenuta successo");
			output.println(" dati file, righe[ "+fileCSV.getRighe().size()+" ], colonne [ "+fileCSV.getColonne().size()+" ] ");
		}
		output.println();
		return fileCSV;
	}
	...
	protected void elaboraFile(CSVFile fileCSV) throws SQLException {
		output.println("--> inizio elaborazione file : "+fileCSV.getNomeFile());
		...
		output.println("");
		output.println("numRigheConErrori : "+numRigheConErrori);
		output.println("numRigheInserite : "+numRigheInserite);
		output.println("numRigheDuplicate : "+numRigheDuplicate);
		output.println("numRigheCancellate : "+numRigheCancellate);
		output.println("--> termine elaborazione file : "+fileCSV.getNomeFile());
	}
